...
<head>
    ...
    <title>Decision Making - Adaptive Competency Survey</title>
...
<body class="min-h-screen bg-slate-950 text-white">
    ...
    <!-- Sticky Header: Title + Progress + Collapsible Instructions -->
    <header class="sticky top-0 z-50 border-b border-white/10 bg-slate-950/70 backdrop-blur">
        <div class="mx-auto max-w-5xl px-4 sm:px-6 py-3">
            ...
            <!-- Collapsible instructions (declutters a LOT) -->
    ...
    <main class="relative mx-auto max-w-5xl px-4 sm:px-6 py-8">
        ...
        <section class="rounded-3xl border border-white/10 bg-white/5 p-4 sm:p-5 backdrop-blur">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-wrap gap-2 text-[11px] text-white/60">
                    <span class="rounded-full border border-white/10 bg-slate-900/30 px-3 py-1">Items 1–5</span>
                    <span class="rounded-full border border-white/10 bg-slate-900/30 px-3 py-1">Decision Making</span>
                </div>
            </div>

            <!-- Optional: keep these, but lighter -->
            <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <div class="rounded-2xl border border-white/10 bg-slate-900/30 px-4 py-3">
                    <p class="text-[11px] uppercase tracking-wider text-white/50 leading-tight">Step</p>
        ...
        <!-- Dimension heading (cleaner, no extra chips) -->
        <section class="mt-8">
            <p class="text-xs uppercase tracking-wider text-white/50">Dimension 1</p>
            <h2 class="mt-1 text-2xl font-extrabold tracking-tight sm:text-3xl">Decision Making</h2>
            <p class="mt-2 text-sm text-white/65">Choose the option that best describes what you would do.</p>
        </section>

        <form method="POST" action="{{ route('survey.dimension1.save') }}" id="dimension1Form" class="mt-6 space-y-5">
            @csrf
        ...
        @php
            ...
            $questions = [
                1 => "When my group is unsure about project instructions, I…",
                2 => "Before an exam that makes me stressed, I…",
                3 => "When someone shares incorrect information about a rule, I…",
                4 => "When managing my weekly allowance, I…",
                5 => "When choosing between two important events, I…",
            ];

            $choices = [
                1 => [
                    ['score' => 2, 'text' => 'ask for clarification before proceeding.'],
                    ['score' => 0, 'text' => 'guess and proceed without checking.'],
                    ['score' => 3, 'text' => 'ask for clarification and help the group summarize the instructions clearly.'],
                    ['score' => 1, 'text' => 'wait for others to clarify.'],
                ],
                2 => [
                    ['score' => 1, 'text' => 'study but without a clear plan.'],
                    ['score' => 3, 'text' => 'create a structured study plan and use stress-management strategies.'],
                    ['score' => 0, 'text' => 'avoid studying until the last minute.'],
                    ['score' => 2, 'text' => 'create a study plan and follow it.'],
                ],
                3 => [
                    ['score' => 1, 'text' => 'correct it briefly without checking accuracy.'],
                    ['score' => 3, 'text' => 'verify, explain clearly, and encourage others to confirm reliable sources.'],
                    ['score' => 0, 'text' => 'ignore it.'],
                    ['score' => 2, 'text' => 'verify and politely share correct information.'],
                ],
                4 => [
                    ['score' => 1, 'text' => 'try to track spending but inconsistently.'],
                    ['score' => 0, 'text' => 'spend without planning.'],
                    ['score' => 3, 'text' => 'budget, monitor expenses, and adjust spending when needed.'],
                    ['score' => 2, 'text' => 'plan a simple budget.'],
                ],
                5 => [
                    ['score' => 1, 'text' => 'ask others to decide for me.'],
                    ['score' => 3, 'text' => 'compare options based on long-term goals and priorities.'],
                    ['score' => 0, 'text' => 'choose randomly or based on convenience.'],
                    ['score' => 2, 'text' => 'compare pros and cons.'],
                ],
            ];
        @endphp
...
document.addEventListener('DOMContentLoaded', () => {
    // Show/hide custom response textarea
    ...
    const form = document.getElementById('dimension1Form');
    ...
    try {
        ...
        if (!res.ok) {
            ...
            const msg = data?.message || 'Saving failed. Please try again.';
            alert(msg);
            btn.disabled = false;
